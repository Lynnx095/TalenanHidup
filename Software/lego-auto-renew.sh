...
check_and_renew_certificate() {
    certFile="./.lego/certificates/_.${DOMAIN_NAME}.crt"

    if [ -f "$certFile" ]; then
        cert_expiry=$(openssl x509 -in "$certFile" -noout -enddate | cut -d'=' -f2)
        cert_expiry_seconds=$(date -d "$cert_expiry" +%s)
        current_time_seconds=$(date +%s)
        days_until_expiry=$(( (cert_expiry_seconds - current_time_seconds) / 86400 ))
        ...
        log "No existing certificate found. Generating a new one..."
        run_lego
        copy_certificate
    fi
}
